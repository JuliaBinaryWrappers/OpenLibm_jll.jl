...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import OpenLibm_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("OpenLibm_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "OpenLibm_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"OpenLibm", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "OpenLibm_jll", "src", "OpenLibm_jll.jl"))
        @info("OpenLibm_ll dev'ed out to /depot/dev/OpenLibm_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("05823500-19ac-5b8b-9628-191a04bc5112"))
platforms = [Pkg.Artifacts.unpack_platform(e, "OpenLibm", artifacts_toml) for e in artifacts["OpenLibm"]]
...
if best_platform === nothing
    @debug("Unable to load OpenLibm; unsupported platform $(triplet(platform_key_abi()))")
else
